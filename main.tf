...
resource "azurerm_app_service" "main" {
  ...
  dynamic "site_config" {
    ...
    content {
      always_on                   = lookup(var.site_config.value, "always_on", false)
      app_command_line            = lookup(var.site_config.value, "app_command_line", null)
      default_documents           = lookup(var.site_config.value, "default_documents", null)
      dotnet_framework_version    = lookup(var.site_config.value, "dotnet_framework_version", null)
      ftps_state                  = lookup(var.site_config.value, "ftps_state", "FtpsOnly")
      health_check_path           = lookup(var.site_config.value, "health_check_path", null)
      number_of_workers           = var.service_plan.per_site_scaling == true ? lookup(var.site_config.value, "number_of_workers") : null
      http2_enabled               = lookup(var.site_config.value, "http2_enabled", false)
      ...
      java_container              = lookup(var.site_config.value, "java_container", null)
      java_container_version      = lookup(var.site_config.value, "java_container_version", null)
      java_version                = lookup(var.site_config.value, "java_version", null)
      local_mysql_enabled         = lookup(var.site_config.value, "local_mysql_enabled", null)
      linux_fx_version            = lookup(var.site_config.value, "linux_fx_version", null)
      windows_fx_version          = lookup(var.site_config.value, "windows_fx_version", null)
      managed_pipeline_mode       = lookup(var.site_config.value, "managed_pipeline_mode", null)
      min_tls_version             = lookup(var.site_config.value, "min_tls_version", null)
      php_version                 = lookup(var.site_config.value, "php_version", null)
      python_version              = lookup(var.site_config.value, "python_version", null)
      remote_debugging_enabled    = lookup(var.site_config.value, "remote_debugging_enabled", null)
      remote_debugging_version    = lookup(var.site_config.value, "remote_debugging_version", null)
      scm_type                    = lookup(var.site_config.value, "scm_type", null)
      use_32_bit_worker_process   = lookup(var.site_config.value, "use_32_bit_worker_process", null)
      websockets_enabled          = lookup(var.site_config.value, "websockets_enabled", null)
