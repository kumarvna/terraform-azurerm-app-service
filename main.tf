...
resource "azurerm_app_service" "main" {
  ...
  dynamic "site_config" {
    ...
    content {
      ...
      ftps_state                  = lookup(each.value, "ftps_state", "FtpsOnly")
      health_check_path           = lookup(each.value, "health_check_path", null)
      number_of_workers           = var.service_plan.per_site_scaling == true ? lookup(each.value, "number_of_workers") : null
      http2_enabled               = lookup(each.value, "http2_enabled", false)
      ...
      java_container              = lookup(each.value, "java_container", null)
      java_container_version      = lookup(each.value, "java_container_version", null)
      java_version                = lookup(each.value, "java_version", null)
      ...
      php_version                 = lookup(each.value, "php_version", null)
      python_version              = lookup(each.value, "python_version", null)
      remote_debugging_enabled    = lookup(each.value, "remote_debugging_enabled", null)
      remote_debugging_version    = lookup(each.value, "remote_debugging_version", null)
      scm_type                    = lookup(each.value, "scm_type", null)
      use_32_bit_worker_process   = lookup(each.value, "use_32_bit_worker_process", null)
      websockets_enabled          = lookup(each.value, "websockets_enabled", null)
